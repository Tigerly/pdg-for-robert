...
void buildActualParameterTrees(CallInst *CI){
  ...
  list<ArgumentWrapper*>::iterator 
    argI = CallWrapper::callMap[CI]->getArgWList().begin(),
    argE = CallWrapper::callMap[CI]->getArgWList().end();
   
  list<ArgumentWrapper*>::iterator 
    argFI = FunctionWrapper::funcMap[callee]->getArgWList().begin(), 
    argFE = FunctionWrapper::funcMap[callee]->getArgWList().end(); 
