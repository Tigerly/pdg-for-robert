...
static void test_encrypt_ecb_verbose(void)
{
    ...
    // 128bit key
    uint8_t __attribute__((annotate("sensitive"))) key[16] =        { (uint8_t) 0x2b, (uint8_t) 0x7e, (uint8_t) 0x15, (uint8_t) 0x16, (uint8_t) 0x28, (uint8_t) 0xae, (uint8_t) 0xd2, (uint8_t) 0xa6, (uint8_t) 0xab, (uint8_t) 0xf7, (uint8_t) 0x15, (uint8_t) 0x88, (uint8_t) 0x09, (uint8_t) 0xcf, (uint8_t) 0x4f, (uint8_t) 0x3c };
    // 512bit text
	...
	for(i = 0; i < 4; ++i)
    {
        AES128_ECB_decrypt(buf + (i*16), key, buf2+(i*16));
        phex(buf2 + (i*16));
    }
}
